...
(() => {
  ...
  const BOT_COST_GROWTH = 1.15;

  const SHIP_CAPACITY_BY_LEVEL = [0, 10, 13, 16, 20, 25];
  const SHIP_UPGRADE_COST_BY_LEVEL = [0, 500, 700, 1000, 1400, 2000];
  const SHIP_LOAD_SECONDS_PER_UNIT = 0.25;
  const SHIP_CYCLE_SECONDS_MIN = 70;
  const SHIP_CYCLE_SECONDS_MAX = 70;

  const GEM_CHANCE_ON_RETURN = 0.12;

  const MINE_UNLOCK_COST_GEMS = 5;
  ...
  class Visual3D {
    ...
    _initGround() {
      const plane = new THREE.Mesh(
        new THREE.PlaneGeometry(this.worldWidth + 500, this.worldHeight + 500),
        new THREE.MeshStandardMaterial({ color: 0x166534, roughness: 1, metalness: 0 })
      );
      plane.rotation.x = -Math.PI / 2;
      plane.position.set(this.worldWidth / 2, -1, this.worldHeight / 2);
      this.scene.add(plane);
    }

    _initPort() {
      const zone = new THREE.Mesh(
        new THREE.CylinderGeometry(58, 58, 2, 32),
        new THREE.MeshStandardMaterial({ color: 0x3b82f6, transparent: true, opacity: 0.12 })
      );
      zone.position.set(this.worldWidth / 2, 0.5, this.worldHeight / 2);
      this.scene.add(zone);
      this.portZoneObject = zone;
    ...
    update(dt) {
      ...
      this.portZoneObject.material.opacity = 0.12 + Math.sin(nowMs() / 200) * 0.06;
    ...
    sync(game, alpha) {
      ...
      this.portZoneObject.position.set(game.port.x, 0.5, game.port.y);
  ...
  class Game {
    ...
    _syncShips() {
      const count = clamp(Number(this.shipCount) || 1, 1, 5);
      this.shipCount = count;
      while (this.ships.length < count) {
        this.ships.push(new Ship(10, this.rng));
      }
    }
    ...
    _syncBots() {
      const desired = clamp(Number(this.botCount) || 1, 1, BOT_MAX);
      this.botCount = desired;
    ...
    shipUpgradeCost() {
      if (this.shipCount >= 5) return null;
      return SHIP_UPGRADE_COST_BY_LEVEL[this.shipCount] || null;
    }
    ...
    _wireUI() {
      const { btnPause, btnReset, buyBot, upgradeShip, unlockMine, boostEfficiency, sellGold } = this.ui;
      ...
      upgradeShip.addEventListener("click", () => {
        const cost = this.shipUpgradeCost();
        if (cost === null) return;
        if (this.money < cost) return;
        this.money -= cost;
        this.shipCount += 1;
    ...
    _load() {
      try {
        const tryKeys = ["ibt_save_v5", "ibt_save_v4", "ibt_save_v3", "ibt_save_v2", "ibt_save_v1"];
        let data = null;
        let version = 0;
        ...
        if (version === 3) {
          ...
          this.shipCount = clamp(Number(data.shipLevel ?? 1) || 1, 1, 5);
        }
        ...
        if (version === 4 || version === 5) {
          ...
          this.portStorage.stone = Number(data.portStorage?.stone ?? 0) || 0;
          this.botCount = clamp(Number(data.botCount ?? 1) || 1, 1, BOT_MAX);
          this.shipCount = clamp(Number(data.shipLevel ?? data.shipCount ?? 1) || 1, 1, 5);
        ...
        if (Array.isArray(data.ships)) {
          ...
          for (const s of data.ships) {
            const ship = new Ship(10, this.rng);
    ...
    syncUI(dt) {
      ...
      shipTimer.textContent = "--:--";
      shipMeta.textContent = `Ships: ${this.shipCount} / 5`;

      const nextBot = this.nextBotCost();
      buyBotCost.textContent = nextBot === null ? "(MAX)" : `(${formatMoney(nextBot)})`;
      buyBot.disabled = nextBot === null || this.money < nextBot;

      const shipUpg = this.shipUpgradeCost();
      upgradeShipCost.textContent = shipUpg === null ? "(MAX)" : `(${formatMoney(shipUpg)})`;
      upgradeShip.disabled = shipUpg === null || this.money < shipUpg;

      unlockMine.disabled = this.mineUnlocked || this.gems < MINE_UNLOCK_COST_GEMS || this.botCount < MINE_MIN_BOTS;
      boostEfficiency.disabled = !this.mineUnlocked || this.gems < EFFICIENCY_BOOST_COST_GEMS;
      sellGold.disabled = Math.floor(this.gold) <= 0;
      ...
      if (this._uiChipCooldown <= 0) {
        ...
        chipShip.textContent = `${this.shipCount} Ships`;
